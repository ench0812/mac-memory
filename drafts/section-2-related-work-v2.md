...
**Agentic file-system memory.** General Agentic Memory (GAM) extends the trend toward structured memory by treating the memory layer as a modular file system with Python SDK, CLI, REST, and Web access modes. This is less a memory representation theory than an operational substrate, but it reinforces the broader shift toward memory as first-class infrastructure rather than a hidden prompt suffix.
